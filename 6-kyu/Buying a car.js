function nbMonths(
  ...
  let save = savingPerMonth;
  let month = 1;
  let available = startPriceNew - (startPriceOld + savingPerMonth);
  let max = available / savingPerMonth + 2;
  let result = [];
  ...
  if (startPriceNew <= startPriceOld) {
    ...
    for (let i = 1; i < max; i++) {
      ...
      available = startPriceNew - startPriceOld - savingPerMonth;
      if (available <= 0) {
        result.push(month);
        result.push(Math.round(Math.abs(available)));
        return result;
      }
      month++;
    }
